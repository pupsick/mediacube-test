    ...
    <div>
        ...
        <form class="my-5" v-if="model">
            ...
            <div class="form-controls mt-4">
                <button type="submit" @click.prevent="updateModel" class="btn btn-primary">Сохранить</button>
                <button type="submit" @click.prevent="deleteModel" class="btn btn-danger">Удалить</button>
            </div>
        </form>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            updateModel() {
                ...
                this.sending = true;
                axios.post('/employees/edit', this.model)
                    .then(response => {
                        this.sending = false;
            ...
            deleteModel() {
                if (confirm(`Вы действительно хотите удалить ${this.model.name} из базы?`)) {
        ...
        created() {
            this.$root.$on("dataFetched", data => {
                ...
                this.model = data.employee;
                this.model.departments = this.model.departments_id
                this.model.patronymic = this.parseName(this.model.name, 2);
                this.model.surname = this.parseName(this.model.name, 1);
                this.model.name = this.parseName(this.model.name, 0);
            });
        }
    }
</script>